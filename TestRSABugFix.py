...
def _find_edges_in_path(path, capacity, no_of_slots):
    ...
    for pair in pairs:
        ...
        if (G[pair[0]][pair[1]]['capacity'] < capacity):
            ...
            # if pair==first_edge:
            #     candi_slots = _find_available_slots(G[pair[0]][pair[1]]['available_slots'], no_of_slots)
...
def _find_available_slots(slots,no_of_slots):
    ...
    if iranges:
        ...
        return cand_slots
def _set_of_pairs(pairs):
    pair_list=[]
    for i in range(0,len(pairs)):
        print(pairs[i])
        pair_list.append(G[pairs[i][0]][pairs[i][1]]['available_slots'])
    return pair_list
def _test_find_edges_in_path(path, capacity, no_of_slots):
    print("selected Path: {}".format(path))
    pairs = [path[i: i + 2] for i in range(len(path) - 1)]
    print(len(pairs))
    set_of_slots=_set_of_pairs(pairs)
    print(set_of_slots)

    count = 0
    allocated_slots = []
    print(pairs)
    common_slots = list(set.intersection(*map(set, set_of_slots)))
    print('slots_in_all {}'.format(common_slots))
    first_edge = pairs[0]
    #candi_slots = _find_available_slots(G[pairs[0][0]][pairs[0][1]]['c'], no_of_slots)
    print(pairs[0][0])
    ...
    for pair in pairs:
        ...
        if (G[pair[0]][pair[1]]['capacity'] < capacity):
            ...
            if count == 0:
                candidate_slots = G[pair[0]][pair[1]]['available_slots'].copy()
                allocated_slots, check_path = (
                    _spec_assign_initial(G[pair[0]][pair[1]], G[pair[0]][pair[1]]['available_slots'], no_of_slots))
                count += 1

            else:
                check_path = _spec_assign(G[pair[0]][pair[1]],
                                          allocated_slots) if allocated_slots is not None else False

    return check_path


def _test_cal_route_assignment(i, j, amt):
    no_of_slots = math.ceil(amt / 12.5)
    paths = nx.shortest_simple_paths(G, source=i, target=j, weight='linkDist')
    req_capacity = no_of_slots * 12.5
    check_continuos_slots = False
    ...
    for a in range(0, 2):
        ...
        find_path = _test_find_edges_in_path(select_path, req_capacity, no_of_slots)
        if (find_path != False):
            assigned_path.append(select_path)
    ...
    if select_path not in assigned_path:
        blocked_connections.append([i, j])
_test_cal_route_assignment('Berlin', 'Ulm', 40)
